...
public void save_loc(File selection) {
  if (selection != null) { 
    String path = selection.getAbsolutePath();
    if (encode_tab) {
      if (!path.endsWith("png")) {
...
public void codeImage(PImage img, String encoded) {
  do {
    int[] pixel = getRandom();
    int c = img.get(pixel[0], pixel[1]);
    int r = (int)red(c);
    int g = (int)green(c);
    int b = (int)blue(c);
    //Modify red
...
public String decodeImage(PImage img) {
  String encoded = "";
  int c;
  do {
    int[] pixel = getRandom();
...
public String cypher(String message, String password) {
  return cypher(message, password, true);
}

public String cypher(String message, String password, boolean scramble) {
...
public String decypher(String message, String password) {
  if (password.length() == 0) return message;
  message = message.toLowerCase();
...
public char cypher(char c, int p) {
...
public char decypher(char c, int p) {
  c = Character.toLowerCase(c);
  if (c == ' ') c = '{';
  if (c == '\n') c = '|';
  int val = ((int)c) - 97;
...
public void setupCoordList(PImage img) {
  coords.clear();
...
public void setupCodes() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "StegTool" };
    if (passedArgs != null) {
